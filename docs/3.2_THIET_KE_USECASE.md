...
| **Precondition** | Tài khoản đã được đăng ký hoặc User có tài khoản Google hợp lệ. |
| **Main Flow** | 1. User truy cập vào giao diện đăng nhập.<br>2. User chọn phương thức đăng nhập (Email/Password hoặc Google OAuth).<br>3. Hệ thống gửi yêu cầu xác thực đến API (`/api/auth/login` hoặc `/api/auth/google`).<br>4. Hệ thống kiểm tra thông tin xác thực (trong DB hoặc verify Google Token).<br>5. Hệ thống tạo JWT Access Token.<br>6. Hệ thống trả về Token và thông tin User.<br>7. Ứng dụng client lưu Token và chuyển hướng vào màn hình chính. |
| **Alternative Flow** | **3a. Sai thông tin đăng nhập:**<br>1. Hệ thống trả về lỗi 401 Unauthorized.<br>2. Hiển thị thông báo lỗi cho User.<br>3. User nhập lại thông tin.<br><br>**3b. Tài khoản Google chưa tồn tại (First time):**<br>1. Hệ thống xác thực Token Google hợp lệ.<br>2. Hệ thống tự động tạo User mới trong database.<br>3. Tiếp tục bước 5 của Main Flow. |
